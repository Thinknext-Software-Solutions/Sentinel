...
Sentinel can call LLMs without knowing which vendor or model is in use.
...
- Errors are wrapped in SentinelLLMError with provider/model context
...
from .exceptions import SentinelLLMError
...
class LLMClient(ABC):
    ...
    def structured_call(
        ...
        *,
        ...
        Raises:
            SentinelLLMError: API failure, timeout, or output that can't be
...
def build_client(
    ...
    *,
    ...
    Raises:
        SentinelLLMError: Unknown provider or initialization failure.
    """
    p = provider.lower()
    if p == "anthropic":
    ...
    if p == "openai":
        ...
        if api_key is None:
            raise SentinelLLMError("OpenAI provider requires an API key")
        return OpenAIClient(api_key=api_key, model=model, base_url=base_url)
    if p == "google":
        from .llm_google import GoogleGeminiClient

        if api_key is None:
            raise SentinelLLMError("Google Gemini provider requires an API key")
