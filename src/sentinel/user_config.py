...
Lives at `~/.config/sentinel/config.yaml` (or $SENTINEL_CONFIG_HOME if set).
...
Precedence (highest wins):
    ...
    2. Project-level sentinel.yaml (handled by config.py / SentinelConfig)
...
def config_dir() -> Path:
    ...
    Honors:
        - $SENTINEL_CONFIG_HOME (explicit override)
        - $XDG_CONFIG_HOME/sentinel (XDG standard)
        - ~/.config/sentinel (XDG default on Linux/Mac)
    """
    explicit = os.environ.get("SENTINEL_CONFIG_HOME")
    if explicit:
...
class UserConfig(BaseModel):
    """The full ~/.config/sentinel/config.yaml structure."""
...
def load_user_config(path: Optional[Path] = None) -> UserConfig:
    ...
    Raises:
        SentinelConfigError: If the file exists but is malformed.
    """
    if path is None:
        path = config_path()
    if not path.exists():
    ...
    try:
        ...
        raise SentinelConfigError(f"Could not read user config {path}: {exc}") from exc
    try:
        data = yaml.safe_load(raw) or {}
    except yaml.YAMLError as exc:
        raise SentinelConfigError(f"Invalid YAML in user config {path}: {exc}") from exc
    ...
    try:
        ...
        raise SentinelConfigError(f"Invalid user config at {path}:\n{exc}") from exc
...
def resolve_llm_credentials(
    *,
    ...
    Raises:
        SentinelConfigError: If the chosen provider requires an API key and
    ...
    if needs_key and not key:
        ...
        hints = [
            f"Set it now: sentinel configure llm {resolved_provider} --key <YOUR_KEY>",
        ...
        if resolved_provider == "anthropic":
            ...
            hints.append(
                ...
                "sentinel configure llm ollama --model llama3.1 --set-default"
            )
        raise SentinelConfigError(
...
def resolve_vcs_credentials(
    *,
    ...
    if not token:
        ...
        hints = [
            f"Set it now: sentinel configure vcs {resolved} --token <YOUR_TOKEN>",
        ]
        if env_names:
        ...
        hints.append(
            f"Or run sentinel build with --no-pr to skip the push and PR step entirely"
        )
        raise SentinelConfigError(
...
def resolve_issue_credentials(
    *,
    ...
    if not token:
        ...
        hints = [
            f"Set it now: sentinel configure issue {provider} --token <YOUR_TOKEN>",
        ]
        if env_names:
            hints.append(f"Or export: export {env_names[0]}=<YOUR_TOKEN>")
        if provider == "jira":
            ...
            hints.append(
                "Or reuse your GitHub VCS token: sentinel configure issue github "
